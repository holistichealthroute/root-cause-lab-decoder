...
const CMB_FIELDS = [
  { label: "CO2", unitOptions: ["mmol/L", "mEq/L"] },
  { label: "BUN", unitOptions: ["mg/dL", "mmol/L"] },
  { label: "Creatinine", unitOptions: ["mg/dL", "µmol/L"] },
  { label: "Albumin", unitOptions: ["g/dL", "g/L"] },
  { label: "TotalProtein", unitOptions: ["g/dL", "g/L"] },
];

const CMB142: React.FC = () => {
  ...
  const [formData, setFormData] = useState(() => {
    const cmb =
      ...
      !Array.isArray(intakeData.cmb)
        ...
        : {};
    return CMB_FIELDS.reduce((acc, field) => {
      const val = cmb[field.label]?.value;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIntakeData((prev: any) => ({
      ...
      cmb: {
        ...(prev.cmb || {}),
        CO2: { value: formData.CO2, unit: units.CO2 },
        BUN: { value: formData.BUN, unit: units.BUN },
        Creatinine: { value: formData.Creatinine, unit: units.Creatinine },
        Albumin: { value: formData.Albumin, unit: units.Albumin },
        TotalProtein: {
          value: formData.TotalProtein,
          unit: units.TotalProtein,
        },
      },
    }));
    navigate("/upload/cmb143");
  };
    ...
    <div className="grid gap-16">
      <section className="card text-align-center">
        ...
        <h2 className="cbc-title">
          Comprehensive Metabolic Panel (CMP-14) - Part 2
        </h2>
